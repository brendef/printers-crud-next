  ...
  const RenderToggleState = () => {
    return ( 
      <label className="relative inline-flex items-center cursor-pointer">
        <input type="checkbox" value="" className="sr-only peer" disabled={!filterByActivityState} checked={showActiveStateOnly} onChange={handleStateToggle} />
        <div className={`w-11 h-6 bg-${filterByActivityState ? 'danger' : 'gray' }-300 rounded-full peer peer-focus:ring-4 peer-focus:ring-success-300 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-${filterByActivityState ? 'success' : 'gray' }-600`}></div>
      </label>
